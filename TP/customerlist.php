...
<div class="container">
    ...
    <a href="mycancellist.php" class="btn" >취소내역</a>

    <a href="mywatchlist.php" class="btn" >관람내역</a>

    <a href="customerlist.php" class="btn btn-primary" >사용자 목록</a>
    </div>
...
$stmt = $conn -> prepare("SELECT CID, NAME, PASSWORD, EMAIL, BIRTH_DATE, SEX, MOVIE_BOOK_ID  FROM CUSTOMER_TP WHERE LOWER(CID) LIKE '%' || :searchWord || '%'  ORDER BY CID");
...
?>
    ...
    $MovieOpenDay = $row['NAME'];
    $IsBooked = $row['EMAIL'];
    $bookId = $row['BIRTH_DATE'];
    $MovieSeats = $row['SEX'];
    $MovieStatus = $row['MOVIE_BOOK_ID'];
